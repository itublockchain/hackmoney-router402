 ...
 */
export async function getToken(args: { chain: string; token: string }): Promise<string> {
 ...
 */
export async function getChains(args: { chainTypes?: string }): Promise<string> {
	...
	const chainTypesSlice = args.chainTypes.split(",").map((ct) => ct.trim().toLowerCase());
...
export async function getConnections(args: {
	...
	const response = await fetch(`${BASE_URL}/v1/connections?${params.toString()}`);
	if (!response.ok) {
 ...
 * Get chain by ID
 */
export async function getChainByIdHandler(args: { id: string }): Promise<string> {
	if (!args.id) {
 ...
 * Get chain by name
 */
export async function getChainByNameHandler(args: { name: string }): Promise<string> {
	if (!args.name) {
