...
export async function getTokenBalance(args: {
	...
	if (!args.rpcUrl || !args.tokenAddress || !args.walletAddress) {
		throw new Error("rpcUrl, tokenAddress, and walletAddress parameters are required");
	}
...
export async function simulateTransactionTool(args: {
	...
	if (args.data) {
		data = (args.data.startsWith("0x") ? args.data : `0x${args.data}`) as `0x${string}`;
	}

	// Parse value if provided
	let value: bigint | undefined;
	if (args.value) {
		value = args.value.startsWith("0x") ? BigInt(args.value) : BigInt(args.value);
	}
