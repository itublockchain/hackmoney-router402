...
export class LiFiMCP extends McpAgent {
	...
	async init() {
		...
		this.server.tool("get-token", { chain: z.string(), token: z.string() }, async (args) =>
			textContent(await lifi.getToken(args)),
		);
		...
		this.server.tool("get-chains", { chainTypes: z.string().optional() }, async (args) =>
			textContent(await lifi.getChains(args)),
		);
		...
		this.server.tool("get-tools", { chains: z.array(z.string()).optional() }, async (args) =>
			textContent(await lifi.getTools(args)),
		);
		...
		this.server.tool(
			...
			async (args) => textContent(await blockchain.simulateTransactionTool(args)),
		);
	}
}
